...
def generate_lookahead_text(model, tokenizer, sequence, n_branch_tokens=5, device='cuda'):
    """
    Wrapper to call get_lookahead_sequences and return detokenized text outputs.
    
    Args:
        model: Hugging Face model with `.generate()` and cache support.
        tokenizer: Corresponding tokenizer.
        sequence (torch.Tensor): Tokenized input with shape (1, seq_len).
        n_branch_tokens (int): Number of top branching tokens.
        device (str): Device to run model on.
        
    Returns:
        list[str]: Detokenized output sequences.
    """
    sequences, _, _ = get_lookahead_sequences(
        model=model,
        tokenizer=tokenizer,
        hypotheses=sequence,
        n_branch_tokens=n_branch_tokens,
        device=device
    )
